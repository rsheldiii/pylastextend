...
class _Network(object):
    ...
    def get_geo(self, lat=None, lon=None, place=None):
        """
            Return a Geo object
        """
	return Geo(self,lat,lon,place)
    ...
    def get_scrobbler(self, client_id, client_version):
        ...
        _deprecation_warning("Use _Network.scrobble(...), _Network.scrobble_many(...), and Network.update_now_playing(...) instead")
    ...
    def create_new_playlist(self, title, description):
        ...
        doc = (self, 'playlist.create', params).execute(False)
...
class Geo(_BaseObject, _Taggable):
    ...
    def __init__(self, network, lat=None, lon=None, place=None):
        """
        Create a geo instance.
        # Parameters:
            * network : The network object calling the geo.
            * lat : latitude; defaults to none, does nothing without lon.
            * lon : longitude; defaults to none, does nothing without lat.
            * place : the last.fm formatted place string. Defaults to none,
                      used instead of the tuple (lat,lon)
            NOTE: Only requires (lat,lon) or place, not both. If both are given,
                  place is ignored.
        """
        
        _BaseObject.__init__(self, network)
        _Taggable.__init__(self, 'Geo')#We don't know what this does yet.
    ...
    def _get_params(self):
        if self.place:
        	return {'location': self.place, }#TODO: imperative, not OO
		return {'lat' : self.lat, 'lon' : self.lon}
        
    def get_upcoming_events(self):
        """Returns a list of the upcoming Events for this Geolocation object.
           Pulls XML for fast initiation. Because seriously, why wouldn't you? seriously though is there something I'm missing? it's all right there
        """
        events = []
        doc = self._request('Geo.getEvents', True)
        for node in doc.getElementsByTagName('event'):
            temp_id = _extract(node, 'id')
            events.append(Event(temp_id, self.network, XML = node))
        return events

	def __repr__(self):
                '''
                   Not currently implemented: Converts Geo object to
...
class Artist(_BaseObject, _Taggable):
    ...
    #returns status of artist's name; if name is wrong, changes artist's name accordingly
        request = self._request("artist.getCorrection", True, {"artist" : self.get_name()})
        if _extract(request,"error") is not None:
        	print "artist is none"
        	return None
        elif _extract(request,"name") is not None:
            self.name = _extract(request,"name")
        return self.name
...
class Event(_BaseObject):
    ...
    def __init__(self, event_id, network, XML = None):
        _BaseObject.__init__(self, network)
    ...
    def get_title(self):
        ...
        if self.getInfoDoc == None:
            self.getInfoDoc = self._request("event.getInfo", True)

        
        return _extract(self.getInfoDoc, "title")
        
    def test(self):
        doc = self.getInfoDoc
        der = doc.getElementsByTagName("venue")
        print der[0]
        print _extract(der[0],"name")
    ...
    def get_artists(self):
        ...
        if not self.getInfoDoc:
            self.getInfoDoc = self._request("event.getInfo", True)
            
            
        names = _extract_all(self.getInfoDoc, "artist")
        artists = []
        for name in names:
    ...
    def get_venue(self):
        ...
        return Venue(venue_id, self.network, v)
    ...
    def get_start_date(self):
        ...
        return _extract(self.getInfoDoc, "startDate")
...
class Venue(_BaseObject):
    ...
    def __init__(self, id, network, XML = None):
        _BaseObject.__init__(self, network)
        if XML:
            self.getInfoDoc = XML
        self.id = _number(id)
    ...
    def get_name(self):
        if self.getInfoDoc:
            return _extract(self.getInfoDoc,"name")
            
            
class Location():
    print "derrrr"
"""location class used specifically for Venue objects and any objects that support location parameters in the form:
city
country
street
postal code
...
def initnetwork():
    ...
    return LastFMNetwork(api_key = API_KEY, api_secret = API_SECRET)
